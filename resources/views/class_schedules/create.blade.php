...
@section('content')
    <div class="container mx-auto px-4 py-6">
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Tambah Jadwal Kelas</h1>
            <p class="text-gray-600 dark:text-gray-400 mt-1">Buat jadwal kelas baru</p>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
            <form action="{{ route('class_schedules.store') }}" method="POST">
                    ...
                    <div>
                        ...
                        <select id="time_slot_id" name="time_slot_id" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('time_slot_id') border-red-500 @enderror" required>
                            ...
                            @foreach ($timeSlots as $timeSlot)
                                <option value="{{ $timeSlot->id }}" @if (old('time_slot_id') == $timeSlot->id) selected @endif>
                ...
                <div class="mt-6">
                    ...
                    <textarea id="notes" name="notes" rows="4" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('notes') border-red-500 @enderror" placeholder="Masukkan catatan tambahan...">{{ old('notes') }}</textarea>
                    @error('notes')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mt-6">
                    <label class="flex items-center">
                        <input type="checkbox" name="is_active" value="1" @if (old('is_active', true)) checked @endif class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500">
                ...
                <div class="flex gap-3 mt-8">
                    <button type="submit" class="inline-flex items-center px-6 py-2 bg-green-600 text-white font-medium rounded-lg hover:bg-green-700 transition">
