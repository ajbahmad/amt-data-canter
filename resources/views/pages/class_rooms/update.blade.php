...
<div class="rounded-2xl border border-slate-50 bg-white p-8">
    ...
    <div class="mb-8 pb-4 border-b border-slate-100">
        ...
        <p class="text-xs text-slate-400 mt-1">Perbarui rombongan belajar dan tetapkan kapasitas daya tampung kelas di bawah ini.</p>
    </div>
    ...
    <form action="{{ route('class_rooms.update', $classRoom->id) }}" method="POST">
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 mb-8">
                ...
                <div class="relative">
                    <select name="school_institution_id" class="w-full text-sm border border-slate-200 rounded-lg focus:border-green-800 focus:ring-4 focus:ring-green-800/10 transition-all duration-200 outline-none text-slate-700 font-medium px-4 py-3 bg-white hover:border-slate-300 appearance-none @error('school_institution_id') border-red-500 @enderror">
                        ...
                        @foreach($schoolInstitutions as $schoolInstitution)
                            <option value="{{ $schoolInstitution->id }}" {{ old('school_institution_id', $classRoom->school_institution_id) == $schoolInstitution->id ? 'selected' : '' }}>{{ $schoolInstitution->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="relative">
                    <select name="grade_id" class="w-full text-sm border border-slate-200 rounded-lg focus:border-green-800 focus:ring-4 focus:ring-green-800/10 transition-all duration-200 outline-none text-slate-700 font-medium px-4 py-3 bg-white hover:border-slate-300 appearance-none @error('grade_id') border-red-500 @enderror">
                        ...
                        @foreach($grades as $grade)
                            <option value="{{ $grade->id }}" {{ old('grade_id', $classRoom->grade_id) == $grade->id ? 'selected' : '' }}>{{ $grade->name }}</option>
                        @endforeach
                    </select>
                    <div class="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none text-slate-400">
                        <iconify-icon icon="lucide:chevron-down" class="text-xs"></iconify-icon>
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $classRoom->name) }}" class="w-full text-sm border border-slate-200 rounded-lg focus:border-green-800 focus:ring-4 focus:ring-green-800/10 transition-all duration-200 outline-none text-slate-700 font-medium px-4 py-3 bg-white hover:border-slate-300 @error('name') border-red-500 @enderror" placeholder="Contoh: 7A">
                @error('name')
                    <p class="text-red-655 text-[10px] mt-1.5 font-bold flex items-center gap-1">
                        <i class="ti ti-alert-circle"></i>{{ $message }}
                    </p>
            ...
            <div>
                ...
                <input type="number" name="capacity" value="{{ old('capacity', $classRoom->capacity) }}" class="w-full text-sm border border-slate-200 rounded-lg focus:border-green-800 focus:ring-4 focus:ring-green-800/10 transition-all duration-200 outline-none text-slate-700 font-medium px-4 py-3 bg-white hover:border-slate-300 @error('capacity') border-red-500 @enderror" placeholder="0">
                @error('capacity')
            ...
            <div class="flex items-end">
                <div class="mb-1 w-full">
                    <label class="flex items-center gap-2.5 cursor-pointer">
                        <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $classRoom->is_active) ? 'checked' : '' }} class="w-4 h-4 rounded text-emerald-600 focus:ring-emerald-500 border-slate-300 transition-all">
