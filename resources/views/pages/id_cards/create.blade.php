...
@section('title', 'Tambah Kartu ID')

@section('content')

@include('layouts.partials.admin.breadcrumb', [
    'title' => 'Tambah Kartu ID',
    'breadcrumbs' => [
    ...
    <form action="{{ route('id_cards.store') }}" method="POST">
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 mb-6">
            ...
            <div>
                ...
                <input type="text" name="card_uid" value="{{ old('card_uid') }}" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 @error('card_uid') border-red-500 @enderror" placeholder="Contoh: UID-ABC123" required>
                @error('card_uid')
            ...
            <div>
                ...
                <input type="text" name="card_number" value="{{ old('card_number') }}" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 @error('card_number') border-red-500 @enderror" placeholder="Contoh: CARD-001234">
                @error('card_number')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="person_id" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 @error('person_id') border-red-500 @enderror" required>
                    ...
                    @foreach($persons as $person)
                        <option value="{{ $person->id }}" {{ old('person_id') == $person->id ? 'selected' : '' }}>{{ $person->full_name }}</option>
                    @endforeach
                </select>
                @error('person_id')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            <div>
                ...
                <input type="date" name="issued_at" value="{{ old('issued_at') }}" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 @error('issued_at') border-red-500 @enderror">
                @error('issued_at')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="date" name="expired_at" value="{{ old('expired_at') }}" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 @error('expired_at') border-red-500 @enderror">
                @error('expired_at')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
