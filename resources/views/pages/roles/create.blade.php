    ...
    <form action="{{ route('roles.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="3"
                ...
                placeholder="Masukkan deskripsi peran">{{ old('description') }}</textarea>
            @error('description')
                <p class="mt-2 text-sm text-red-500"><i class="ti ti-alert-circle mr-1"></i>{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <select id="scope" name="scope" required
                ...
                <option value="global" {{ old('scope') == 'global' ? 'selected' : '' }}>Global (Semua Akses)</option>
                <option value="institution" {{ old('scope') == 'institution' ? 'selected' : '' }}>Lembaga (Spesifik Lembaga)</option>
                <option value="school" {{ old('scope') == 'school' ? 'selected' : '' }}>Sekolah (Spesifik Tingkat)</option>
            </select>
            @error('scope')
                <p class="mt-2 text-sm text-red-500"><i class="ti ti-alert-circle mr-1"></i>{{ $message }}</p>
            @enderror
            ...
            <div id="institution-field" style="display: none;">
                ...
                <select id="school_institution_id" name="school_institution_id"
                    ...
                    @foreach($institutions as $inst)
                        <option value="{{ $inst->id }}" {{ old('school_institution_id') == $inst->id ? 'selected' : '' }}>
                            {{ $inst->name }}
                        </option>
                    @endforeach
                </select>
