...
<div class="rounded-2xl border border-slate-50 bg-white p-8">
    ...
    <div class="mb-8 pb-4 border-b border-slate-100">
        ...
        <p class="text-xs text-slate-400 mt-1">Lengkapi informasi biodata dan detail pendaftaran siswa baru di bawah ini.</p>
    </div>
    ...
    <form action="{{ route('students.store') }}" method="POST">
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 mb-6">
            ...
            <div>
                ...
                <div class="relative">
                    <select name="person_id" class="w-full text-sm border border-slate-200 rounded-lg focus:border-green-800 focus:ring-4 focus:ring-green-800/10 transition-all duration-200 outline-none text-slate-700 font-medium px-4 py-3 bg-white hover:border-slate-300 @error('person_id') border-red-500 @enderror">
            ...
            <div>
                ...
                <input type="text" name="student_id" value="{{ old('student_id') }}" class="w-full text-sm border border-slate-200 rounded-lg focus:border-green-800 focus:ring-4 focus:ring-green-800/10 transition-all duration-200 outline-none text-slate-700 font-medium px-4 py-3 bg-white hover:border-slate-300 @error('student_id') border-red-500 @enderror" placeholder="Contoh: STU-00001">
                @error('student_id')
                    <p class="text-red-600 text-[10px] mt-1.5 font-bold flex items-center gap-1">
                        <i class="ti ti-alert-circle"></i>{{ $message }}
                    </p>
            ...
            <div>
                ...
                <input type="text" name="enrollment_number" value="{{ old('enrollment_number') }}" class="w-full text-sm border border-slate-200 rounded-lg focus:border-green-800 focus:ring-4 focus:ring-green-800/10 transition-all duration-200 outline-none text-slate-700 font-medium px-4 py-3 bg-white hover:border-slate-300 @error('enrollment_number') border-red-500 @enderror" placeholder="Nomor pendaftaran">
            ...
            <div>
                ...
                <input type="date" name="enrollment_date" value="{{ old('enrollment_date') }}" class="w-full text-sm border border-slate-200 rounded-lg focus:border-green-800 focus:ring-4 focus:ring-green-800/10 transition-all duration-200 outline-none text-slate-700 font-medium px-4 py-3 bg-white hover:border-slate-300 @error('enrollment_date') border-red-500 @enderror">
            ...
            <div>
                ...
                <div class="relative">
                    <select name="status" class="w-full text-sm border border-slate-200 rounded-lg focus:border-green-800 focus:ring-4 focus:ring-green-800/10 transition-all duration-200 outline-none text-slate-700 font-medium px-4 py-3 bg-white hover:border-slate-300 appearance-none @error('status') border-red-500 @enderror">
                        ...
                        <option value="dropped_out" {{ old('status') === 'dropped_out' ? 'selected' : '' }}>Putus Sekolah</option>
                        <option value="suspended" {{ old('status') === 'suspended' ? 'selected' : '' }}>Ditunda</option>
                    </select>
                    <div class="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none text-slate-400">
                        <iconify-icon icon="lucide:chevron-down" class="text-xs"></iconify-icon>
                    </div>
