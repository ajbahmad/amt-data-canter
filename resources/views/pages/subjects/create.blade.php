...
<div class="rounded-2xl border border-slate-50 bg-white p-8">
    ...
    <div class="mb-8 pb-4 border-b border-slate-100">
        ...
        <p class="text-xs text-slate-400 mt-1">Daftarkan kurikulum mata pelajaran (mapel) baru beserta kode mapel dan keterhubungannya dengan level sekolah.</p>
    </div>
    ...
    <form action="{{ route('subjects.store') }}" method="POST">
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 mb-6">
            ...
            <div>
                ...
                <input type="text" name="code" value="{{ old('code') }}" class="w-full text-sm border border-slate-200 rounded-lg focus:border-green-800 focus:ring-4 focus:ring-green-800/10 transition-all duration-200 outline-none text-slate-700 font-medium px-4 py-3 bg-white hover:border-slate-300 @error('code') border-red-500 @enderror" placeholder="Contoh: MTK">
                @error('code')
                    <p class="text-red-655 text-[10px] mt-1.5 font-bold flex items-center gap-1">
                        <i class="ti ti-alert-circle"></i>{{ $message }}
                    </p>
