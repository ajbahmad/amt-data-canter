...
@include('layouts.partials.admin.breadcrumb', [
    'title' => 'Edit Penugasan Guru Mapel',
    'breadcrumbs' => [
...
<div class="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6">
    <form action="{{ route('teacher_subject_assignments.update', $teacherSubjectAssignment) }}" method="POST">
        @csrf
        @method('PUT')
                ...
                <select name="school_institution_id"
                    ...
                    @foreach ($schoolInstitutions as $schoolInstitution)
                        <option value="{{ $schoolInstitution->id }}"
                            {{ old('school_institution_id', $teacherSubjectAssignment->semester->school_institution_id) == $schoolInstitution->id ? 'selected' : '' }}>
                ...
                <select name="school_level_id"
                    ...
                    @foreach ($schoolLevels as $schoolLevel)
                        <option value="{{ $schoolLevel->id }}"
                            {{ old('school_level_id', $teacherSubjectAssignment->semester->schoolYear->school_level_id) == $schoolLevel->id ? 'selected' : '' }}>{{ $schoolLevel->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="class_room_id" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 @error('class_room_id') border-red-500 @enderror">
                    ...
                    @foreach($classRooms as $room)
                        <option value="{{ $room->id }}" {{ old('class_room_id', $teacherSubjectAssignment->class_room_id) == $room->id ? 'selected' : '' }}>
                            {{ $room->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="semester_id" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 @error('semester_id') border-red-500 @enderror">
                    ...
                    @foreach($semesters as $semester)
                        <option value="{{ $semester->id }}" {{ old('semester_id', $teacherSubjectAssignment->semester_id) == $semester->id ? 'selected' : '' }}>
                            {{ $semester->name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 mb-6">
            <div>
                ...
                <select name="subject_id" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 @error('subject_id') border-red-500 @enderror">
                    ...
                    @foreach($subjects as $subject)
                        <option value="{{ $subject->id }}" {{ old('subject_id', $teacherSubjectAssignment->subject_id) == $subject->id ? 'selected' : '' }}>
                            {{ $subject->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="teacher_id" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 @error('teacher_id') border-red-500 @enderror">
                    ...
                    @foreach($teachers as $teacher)
                        <option value="{{ $teacher->id }}" {{ old('teacher_id', $teacherSubjectAssignment->teacher_id) == $teacher->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="datetime-local" name="assigned_at" value="{{ old('assigned_at', $teacherSubjectAssignment->assigned_at->format('Y-m-d\TH:i')) }}" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 @error('assigned_at') border-red-500 @enderror">
            ...
            <div class="mt-5 pt-5">
                <label class="flex items-center text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $teacherSubjectAssignment->is_active) ? 'checked' : '' }} class="rounded dark:bg-gray-700">
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('teacher_subject_assignments.show', $teacherSubjectAssignment) }}" class="inline-flex items-center rounded-lg bg-gray-300 dark:bg-gray-700 px-6 py-2 text-gray-800 dark:text-gray-300 hover:bg-gray-400 dark:hover:bg-gray-600 transition">
